...
truth for copy. Shares its architecture with LP1 and runs on the same Meta pixel,
since both pages belong to one campaign.
...
| `PUBLIC_META_PIXEL_ID` | Meta Pixel | **supplied** — `28341044768871070`, same as LP1 |
...
Every lead carries `landing_page: "LP2"`, so LP1 and LP2 stay separable in the
CRM and in reporting even though they share a pixel.
...
Cloudflare Pages, connected to `TeamFeyth/-Kayalar-LP2`:
...
Sections come from Section 5 of the brief, in this order:

1. Hero — headline, value prop, View Inventory + Call, lead form
2. Our Promise to Every Customer — five bullets
3. Ready when you are — the six featured vehicles
4. Family Owned. Built on Trust. — brand story
5. Not sure which car is right? — contact block + Lead Form 2
6. Footer

The rough draft also contained "How Buying at Kayalar Works", "Shop By Body
Type", an FAQ and a footer nav. None of those appear in Section 5 and none have
approved copy, so they are not built. They are additive if the team wants them
later — send copy and link targets.

### One ambiguity, and how it is handled

Section 3 defines a confirmation message for a **hero form** on LP2, but Section
5.2 lists only scroll and call buttons for the hero. The hero ships **with** the
form, since Section 3 is explicit that one exists and a paid-traffic page wants a
conversion point above the fold. `PUBLIC_HERO_FORM=false` removes it and leaves
the hero on the two CTAs, if the team decides the other way.
...
5.7 turns into on the CRM side.
...
**Meta events.** `PageView` on load, `Lead` on submit, `Contact` on any `tel:`
click. The browser `Lead` and the server CAPI `Lead` share an `event_id`, so they
de-duplicate correctly the moment `META_CAPI_TOKEN` is added.
...
not machine-swapped at runtime: `/` and `/es/`, with `hreflang` tags and a toggle
that keeps the visitor on the same page.
...
exposes one. LP1 carries the same six vehicles, so both files need the same edit.
